                        ...
                        <select name="kode_buku" id="kode_buku"
                            ...
                            @foreach ($buku as $item)
                                <option value="{{ $item->kode }}"
                                    {{ isset($peminjaman) && $peminjaman->kode_buku == $item->kode ? 'selected' : '' }}>
                                    {{ $item->judul }}</option>
                            @endforeach
                        </select>
                        @error('kode_buku')
                        ...
                        <select name="id_anggota" id="id_anggota"
                            ...
                            @foreach ($anggota as $item)
                                <option value="{{ $item->id }}"
                                    {{ isset($peminjaman) && $peminjaman->id_anggota == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama }}</option>
                            @endforeach
                        </select>
                        @error('id_anggota')
                    ...
                    @if (isset($peminjaman))
                        <div class="form-group form-check">
                            <input type="checkbox" name="status"
                                class="form-check-input @error('status') is-invalid @enderror" id="status">
